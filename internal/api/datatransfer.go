...
func (s *Store) handleExportData(w http.ResponseWriter, r *http.Request, owner string) {
	...
	json.NewDecoder(r.Body).Decode(&req) // best-effort — an empty/missing body just means no local prefs to carry
	...
	for rows.Next() {
		...
		if rows.Scan(&f.MessageID, &f.Score, &f.Reasons, &f.SPF, &f.DKIM, &f.DMARC, &f.CreatedAt) == nil {
...
func (s *Store) handleImportData(w http.ResponseWriter, r *http.Request, owner string) {
	...
	rows, err = s.db.Query("SELECT id, name FROM tags WHERE owner_subject = $1", owner)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	for _, t := range b.Tags {
		if id, ok := tagIDByName[t.Name]; ok {
			s.db.Exec("UPDATE tags SET color = $1, notify = $2, instant_move = $3 WHERE id = $4",
	...
	for _, mt := range b.MessageTags {
		...
		s.db.Exec("INSERT INTO message_tags (message_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", mt.MessageID, tagID)
	}
	...
	for _, email := range b.TrustedSenders {
		s.db.Exec("INSERT INTO trusted_senders (owner_subject, sender_email) VALUES ($1, $2) ON CONFLICT DO NOTHING", owner, email)
	}
	...
	if b.OwnerSettings.AutoTagMode != "" {
		s.db.Exec(`
	...
	// apply them itself instead of duplicating that list of keys server-side.
	writeJSON(w, map[string]any{"localPreferences": b.LocalPreferences})
}
